...
static RTEXITCODE generateOutputInnerX86AndAMD64(FILE *pOutput)
{
    ...
    for (PMYEXPORT pExp = g_pExpHead; pExp; pExp = pExp->pNext)
        if (pExp->fData)
            fprintf(pOutput,
                    ...
                    "    mov   xAX, [RT_WRT_RIP(NAME(g_LazyPtr_%s))]\n"
                    "    test  xAX, xAX\n"
                    "    jz    ___LazyLoad___%s\n"
                    "    ret\n"
            ...
            fprintf(pOutput,
                    "BEGINPROC %s\n"
                    "    jmp   RTCCPTR_PRE [RT_WRT_RIP(NAME(g_pfn%s))]\n"
                    "ENDPROC   %s\n",
    ...
    for (PMYEXPORT pExp = g_pExpHead; pExp; pExp = pExp->pNext)
    {
        if (!pExp->fNoName)
            fprintf(pOutput,
                    ...
                    /* "int3\n" */
                    "%%ifdef RT_ARCH_AMD64\n"
                    "    lea     rax, [RT_WRT_RIP(g_sz%s)]\n"
                    "    lea     r10, [RT_WRT_RIP(NAME(%s%s))]\n"
    ...
    if (!g_fSystemLibrary)
        fprintf(pOutput,
                ...
                "    lea     rdx, [RT_WRT_RIP(g_hMod)]      ; phLdrMod\n"
                "    lea     rcx, [RT_WRT_RIP(g_szLibrary)] ; pszFilename\n"
                "    sub     rsp, 28h\n"
                "    call    IMP2(SUPR3HardenedLdrLoadAppPriv)\n"
                "    add     rsp, 28h\n"
                "\n"
        ...
        fprintf(pOutput,
                ...
                "\n"
                "%%elifdef ASM_CALL64_MSC\n"
                "    lea     r8, [RT_WRT_RIP(g_hMod)]       ; phLdrMod\n"
    ...
    fprintf(pOutput,
            ...
            "%%ifdef ASM_CALL64_GCC\n"
            "    mov     edx, eax\n"
            "    lea     rsi, [RT_WRT_RIP(g_szLibrary)]\n"
            "    lea     rdi, [RT_WRT_RIP(g_szFailLoadFmt)]\n"
            "    sub     rsp, 08h\n"
            "%%elifdef ASM_CALL64_MSC\n"
            "    mov     r8d, eax\n"
            "    lea     rdx, [RT_WRT_RIP(g_szLibrary)]\n"
            "    lea     rcx, [RT_WRT_RIP(g_szFailLoadFmt)]\n"
            "    sub     rsp, 28h\n"
            "%%elifdef RT_ARCH_X86\n"
            "    push    eax\n"
            "    push    g_szLibrary\n"
    ...
    fprintf(pOutput,
            ...
            "    lea     rdx, [RT_WRT_RIP(g_szLibrary)]\n"
            "    mov     rsi, r15\n"
            "    lea     rdi, [RT_WRT_RIP(g_szFailResolveFmt)]\n"
            "    sub     rsp, 08h\n"
            "%%elifdef ASM_CALL64_MSC\n"
            "    mov     r9d, eax\n"
            "    mov     r8, r15\n"
            "    lea     rdx, [RT_WRT_RIP(g_szLibrary)]\n"
            "    lea     rcx, [RT_WRT_RIP(g_szFailResolveFmt)]\n"
            "    sub     rsp, 28h\n"
            "%%elifdef RT_ARCH_X86\n"
            "    push    eax\n"
            "    push    dword [ebp + 12]\n"
    ...
    if (g_fWithExplictLoadFunction)
    {
        ...
        fprintf(pOutput,
                ...
                "    ;\n"
                "    cmp     RTCCPTR_PRE [RT_WRT_RIP(g_hMod)], 0\n"
                "    jnz     .loaded\n"
                "\n"
                "    ;\n"
                "    ; Load the module.\n"
        ...
        if (!g_fSystemLibrary)
         ...
         fprintf(pOutput,
                ...
                "    je      .return\n"
                "\n"
                "    lea     pszCurStr, [RT_WRT_RIP(g_szzNames)]\n"
                "    lea     xBX, [RT_WRT_RIP(g_apfnImports)]\n"
                ".next_import:\n"
                ...
                "    mov     rdi, [RT_WRT_RIP(g_hMod)] ; hLdrMod\n"
                " %%ifdef IN_RT_R3\n"
                "    call    NAME(RTLdrGetSymbol)\n"
                " %%else\n"
                "    call    IMP2(RTLdrGetSymbol)\n"
                ...
                "    mov     rcx, [RT_WRT_RIP(g_hMod)] ; pszSymbol\n"
                " %%ifdef IN_RT_R3\n"
                "    call    NAME(RTLdrGetSymbol)\n"
                " %%else\n"
                "    call    IMP2(RTLdrGetSymbol)\n"
