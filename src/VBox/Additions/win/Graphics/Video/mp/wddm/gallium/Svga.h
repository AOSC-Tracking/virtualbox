...
{
    VMSVGACB_INVALID = 0,
    VMSVGACB_MINIPORT = 1,                         /* Commands generated by the miniport. */
    VMSVGACB_UMD = 2                               /* WDDM DMA buffer. */
} VMSVGACBTYPE;
...
/* Layout of physical page for SVGA_CB_CONTEXT_0 CBHeader ring buffer and
 * for SVGA_CB_CONTEXT_DEVICE CBHeader and data.
 */
typedef struct VMSVGACBHEADERS
{
    SVGACBHeader         aContext0CBHeaders[SVGA_CB_MAX_QUEUED_PER_CONTEXT]; /* Ring buffer. */
    SVGACBHeader         contextDeviceCBHeader;
    uint8_t              au8ContextDeviceData[4096 - sizeof(SVGACBHeader) - sizeof(SVGACBHeader) * SVGA_CB_MAX_QUEUED_PER_CONTEXT];
} VMSVGACBHEADERS, *PVMSVGACBHEADERS;
AssertCompile(sizeof(VMSVGACBHEADERS) == 4096);
...
typedef struct VMSVGACBCONTEXT
{
    ...
    uint32_t             idxNextHeader;            /* Index in SVGACBHeader ring buffer. */
    uint32_t             cHeaders;                 /* Number of free elements in the ring buffer. */
} VMSVGACBCONTEXT, *PVMSVGACBCONTEXT;

typedef struct VMSVGACBLOOKASIDELIST
{
    RTLISTANCHOR         ListCB;                   /* VMSVGACB structures which can be reused. */
    uint32_t             cEntries;                 /* Current number of entries in the list. */
    uint32_t             cMinEntries;              /* Keep at least so many entries in the list. */
    uint32_t             cMaxEntries;              /* Maximum number of entries which are allowed to be in the list. */
    uint32_t             u32MissRate;              /* Exponential moving average of miss rate. */
} VMSVGACBLOOKASIDELIST, *PVMSVGACBLOOKASIDELIST;

typedef struct VMSVGACBSTATE
{
    VMSVGACBCONTEXT      aCBContexts[1];           /* Currently only SVGA_CB_CONTEXT_0 is used. */
    VMSVGACBPAGE         CBHeadersPage;            /* Memory for VMSVGACBHEADERS */
    ...
    VMSVGACBLOOKASIDELIST ListCB;                   /* VMSVGACB structures (VMSVGACB_MINIPORT) which can be reused. */
    VMSVGACBLOOKASIDELIST ListCBUMD;                /* VMSVGACB structures (VMSVGACB_UMD) which can be reused. */
} VMSVGACBSTATE, *PVMSVGACBSTATE;

DECLINLINE(PVMSVGACBHEADERS) cbStateHeaders(PVMSVGACBSTATE pCBState)
{
    return (PVMSVGACBHEADERS)pCBState->CBHeadersPage.pvR0;
}

DECLINLINE(PHYSICAL_ADDRESS) cbStateHeadersPA(PVMSVGACBSTATE pCBState, uint32_t off)
{
    Assert(off < sizeof(VMSVGACBHEADERS));
    Assert((off & 0x3f) == 0);
    PHYSICAL_ADDRESS pa;
    pa.QuadPart = pCBState->CBHeadersPage.PhysAddr + off;
    return pa;
}
