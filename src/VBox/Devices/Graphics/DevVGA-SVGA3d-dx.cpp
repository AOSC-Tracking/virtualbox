...
int vmsvga3dDXDestroyContext(PVGASTATECC pThisCC, uint32_t cid)
{
    ...
    RTMemFree(pDXContext->cot.paElementLayout);
    RTMemFree(pDXContext->cot.paBlendState);
    RTMemFree(pDXContext->cot.paDepthStencil);
    RTMemFree(pDXContext->cot.paRasterizerState);
    RTMemFree(pDXContext->cot.paSampler);
    RTMemFree(pDXContext->cot.paStreamOutput);
    RTMemFree(pDXContext->cot.paQuery);
    RTMemFree(pDXContext->cot.paShader);
    RTMemFree(pDXContext->cot.paUAView);
    RTMemFree(pDXContext->cot.paVideoProcessor);
    RTMemFree(pDXContext->cot.paVideoDecoderOutputView);
...
#else
    if (pDXContext->svgaDXContext.inputAssembly.layoutId != elementLayoutId)
    {
        pDXContext->svgaDXContext.inputAssembly.layoutId = elementLayoutId;
...
#else
    if (pDXContext->svgaDXContext.inputAssembly.topology != (uint32)topology)
    {
        pDXContext->svgaDXContext.inputAssembly.topology = topology;
...
int vmsvga3dDXSetRenderTargets(PVGASTATECC pThisCC, uint32_t idDXContext, SVGA3dDepthStencilViewId depthStencilViewId, uint32_t cRenderTargetViewId, SVGA3dRenderTargetViewId const *paRenderTargetViewId)
{
...
#else
    bool fModified = false;
    if (pDXContext->svgaDXContext.renderState.depthStencilViewId != depthStencilViewId)
    {
        pDXContext->svgaDXContext.renderState.depthStencilViewId = depthStencilViewId;
        fModified = true;
    }

    uint32_t cRenderTargets = 0;
    for (uint32_t i = 0; i < RT_ELEMENTS(pDXContext->svgaDXContext.renderState.renderTargetViewIds); ++i)
    {
        SVGA3dRenderTargetViewId const rtvid = i < cRenderTargetViewId ? paRenderTargetViewId[i] : SVGA3D_INVALID_ID;
        if (pDXContext->svgaDXContext.renderState.renderTargetViewIds[i] != rtvid)
        {
            pDXContext->svgaDXContext.renderState.renderTargetViewIds[i] = rtvid;
            fModified = true;
        }

        if (rtvid != SVGA3D_INVALID_ID)
            cRenderTargets = i + 1;
    }
    ...
    /* Micro-optimization, remember how many slot the context actually uses. */
    pDXContext->cRenderTargets = cRenderTargets;
...
#ifdef DX_STATE_TRACKER
    /* Cleanup the element layout. */
    pSvgaR3State->pFuncsDX->pfnDXDestroyElementLayout(pThisCC, pDXContext, elementLayoutId);

    /* If the current element layout is redefined, then tell the backend to re-apply it. */
    if (pDXContext->svgaDXContext.inputAssembly.layoutId == elementLayoutId)
        pDXContext->u64ContextFlags |= DX_CTX_F_STATE_INPUTLAYOUT;
#endif
...
#ifdef DX_STATE_TRACKER
    /* If the current element layout is destroyed, then unset it. */
    if (pDXContext->svgaDXContext.inputAssembly.layoutId == elementLayoutId)
    {
        pDXContext->svgaDXContext.inputAssembly.layoutId = SVGA3D_INVALID_ID;
        pDXContext->u64ContextFlags |= DX_CTX_F_STATE_INPUTLAYOUT;
    }
#endif
    return rc;
...
static int dxBindShader(DXShaderInfo *pShaderInfo, void const *pvShaderBytecode, uint32_t cbShaderBytecode)
{
    AssertReturn(cbShaderBytecode >= 8, VERR_INTERNAL_ERROR); /* Host ensures this in DefineShader. */

    int rc = DXShaderParse(pvShaderBytecode, cbShaderBytecode, pShaderInfo);
    if (RT_SUCCESS(rc))
    {
        ...
        ASSERT_GUEST_RETURN(cToken <= cbShaderBytecode / 4, VERR_INVALID_PARAMETER);
         ...
         */
        uint32_t cbSignaturesAvail = cbShaderBytecode - cToken * 4; /* How many bytes for signatures are available. */
...
int vmsvga3dDXBindShader(PVGASTATECC pThisCC, SVGA3dCmdDXBindShader const *pCmd, PVMSVGAMOB pMob)
{
    ...
    {
        /*
         * Read the shader bytecode and optional signatures from the mob.
         */
        uint32_t const cbMob = vmsvgaR3MobSize(pMob);

        ASSERT_GUEST_RETURN(pEntry->offsetInBytes < cbMob, VERR_INVALID_PARAMETER);
        ASSERT_GUEST_RETURN(pEntry->sizeInBytes <= cbMob - pEntry->offsetInBytes, VERR_INVALID_PARAMETER);

        void *pvShaderBytecode = RTMemTmpAlloc(pEntry->sizeInBytes);
        AssertReturn(pvShaderBytecode, VERR_NO_MEMORY);

        rc = vmsvgaR3MobRead(pSvgaR3State, pMob, pEntry->offsetInBytes, pvShaderBytecode, pEntry->sizeInBytes);
        AssertRCReturnStmt(rc, RTMemTmpFree(pvShaderBytecode), rc);

        /* Fetch the shader and optional signatures. */
        DXShaderInfo shaderInfo;
        RT_ZERO(shaderInfo);
        rc = dxBindShader(&shaderInfo, pvShaderBytecode, pEntry->sizeInBytes);
        if (RT_SUCCESS(rc))
        {
                              ...
                              SVGACOTableType enmType, uint32_t validSizeInBytes, bool fGrow)
{
    ...
    *     pMob is the new COTable. validSizeInBytes is how many entries in the MOB are already defined.
    *     Create a new host buffer, copy validSizeInBytes from the MOB;
    *   2) pMob != NULL, fGrow = true:
    *     pMob is the new COTable. validSizeInBytes is how many entries in the MOB should be already defined
    *     in the currently bound, old COtable MOB.
    *     Create a new host buffer, copy validSizeInBytes from the old mob;
    *   3) pMob == NULL:
    *     Delete the COTable, deallocate the host buffer.
    */
    ...
    if (pMob)
    {
        ...
        pvCOT = RTMemAllocZ(cbCOT);
        AssertReturn(pvCOT, VERR_NO_MEMORY);
    }
    else
    {
        cEntries = 0;
...
#   undef CASE_DX_COT
    ...
    if (cValidEntries)
    {
        if (fGrow)
        {
            /* Copy entries from the old host buffer to the new host buffer.
             * The GROW command is used only to add new empty entries to an existing COTable.
             */
        ...
        else
        {
            ...
            int rc2 = vmsvgaR3MobRead(pSvgaR3State, pMob, 0, pvCOT, validSizeInBytes);
            AssertRC(rc2);
            if (RT_FAILURE(rc2))
...
#ifdef DX_STATE_TRACKER
            if (enmType == SVGA_COTABLE_ELEMENTLAYOUT)
                pDXContext->u64ContextFlags |= DX_CTX_F_STATE_INPUTLAYOUT;
            else if (   enmType == SVGA_COTABLE_RTVIEW
                     || enmType == SVGA_COTABLE_DSVIEW)
                pDXContext->u64ContextFlags |= DX_CTX_F_STATE_RENDERTARGET;
    ...
    rc = pSvgaR3State->pFuncsDX->pfnDXSetCOTable(pThisCC, pDXContext, enmType, cValidEntries, fGrow);
...
#   define CASE_DX_COT(_COTName, _COTEntryType, _COTField) \
        case _COTName: \
            pvCOT = pDXContext->cot.pa ## _COTField; \
            cbCOT = pDXContext->cot.c ## _COTField * sizeof(_COTEntryType); \
            break;
        CASE_DX_COT(SVGA_COTABLE_RTVIEW,           SVGACOTableDXRTViewEntry,                     RTView)
        CASE_DX_COT(SVGA_COTABLE_DSVIEW,           SVGACOTableDXDSViewEntry,                     DSView)
        CASE_DX_COT(SVGA_COTABLE_SRVIEW,           SVGACOTableDXSRViewEntry,                     SRView)
...
int vmsvga3dDXSetUAViews(PVGASTATECC pThisCC, uint32_t idDXContext, SVGA3dCmdDXSetUAViews const *pCmd, uint32_t cUAViewId, SVGA3dUAViewId const *paUAViewId)
{
    ...
    {
        SVGA3dUAViewId const uaViewId = paUAViewId[i];
        if (pDXContext->svgaDXContext.uaViewIds[i] != uaViewId)
        {
            pDXContext->svgaDXContext.uaViewIds[i] = uaViewId;
            fModified = true;
        }
...
int vmsvga3dDXSetCSUAViews(PVGASTATECC pThisCC, uint32_t idDXContext, SVGA3dCmdDXSetCSUAViews const *pCmd, uint32_t cUAViewId, SVGA3dUAViewId const *paUAViewId)
{
    ...
    {
        SVGA3dUAViewId const uaViewId = paUAViewId[i];
        if (pDXContext->svgaDXContext.csuaViewIds[pCmd->startIndex + i] != uaViewId)
        {
            pDXContext->svgaDXContext.csuaViewIds[pCmd->startIndex + i] = uaViewId;
            fModified = true;
